...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String> cars= new ArrayList<>();//her okuttuğu değeri bir liste at
        List<String> list= new ArrayList<>();
        List<String> year= new ArrayList<>();
        List<String> sortedList= new ArrayList<>();
        
        try (Stream<String> stream= Files.lines(Paths.get("src/test/java/araba/"+fileName))){
            ...
            //startsWith ile o yazı ile başlayanları al diyoruz
        }catch (IOException e){
            e.printStackTrace();
        }
        ...
        for (int i=0; i<cars.size(); i=i+4) {
            Car record=new Car(cars.get(i).substring(7),
                                cars.get(i+1).substring(8),
                                Integer.parseInt(cars.get(i+2).substring(7)),
                                //stringi intigere çeviriyoruz
